...
{props.Courses.data.map((items)=>{
    const {title,desc,date} = items.attributes;  
...
export async function getServerSideProps(context) {
    ...
    let a = await fetch("http://localhost:1337/api/courses",{headers:headers});
    let courses = await a.json();
